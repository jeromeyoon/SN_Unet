...
class networks(object):
	...
	dec1 = tf.nn.dropout(g_bn8(deconv2d(enc7,[self.batch_size,4,4,self.df_dim*8],name='g_dec1')),keep_prob)#output 4x4
	dec1 = tf.nn.relu(tf.concat(3,[dec1,enc6]))
	g_bn9 = batch_norm(self.batch_size,name='g_bn9')
	dec2 = tf.nn.dropout(g_bn9(deconv2d(dec1,[self.batch_size,8,8,self.df_dim*8],name='g_dec2')),keep_prob)#output 8x8
	dec2 = tf.nn.relu(tf.concat(3,[dec2,enc5]))
	g_bn10 = batch_norm(self.batch_size,name='g_bn10')
	dec3 = tf.nn.dropout(g_bn10(deconv2d(dec2,[self.batch_size,16,16,self.df_dim*8],name='g_dec3')),keep_prob)#output 16x16
	dec3 = tf.nn.relu(tf.concat(3,[dec3,enc4]))
	g_bn11 = batch_norm(self.batch_size,name='g_bn11')
	dec4 = g_bn11(deconv2d(dec3,[self.batch_size,32,32,self.df_dim*8],name='g_dec4'))#output 32x32
	dec4 = tf.nn.relu(tf.concat(3,[dec4,enc3]))
	g_bn12 = batch_norm(self.batch_size,name='g_bn12')
	dec5 = g_bn12(deconv2d(dec4,[self.batch_size,64,64,self.df_dim*4],name='g_dec5'))#output 64x64
	dec5 = tf.nn.relu(tf.concat(3,[dec5,enc2]))
	g_bn13 = batch_norm(self.batch_size,name='g_bn13')
	dec6 = g_bn13(deconv2d(dec5,[self.batch_size,128,128,self.df_dim*2],name='g_dec6'))#output 128x128
	dec6 = tf.nn.relu(tf.concat(3,[dec6,enc1]))
	dec7 = deconv2d(dec6,[self.batch_size,256,256,3],name='g_dec7')#output 256x256
	...
	dec4 = tf.nn.relu(tf.concat(3,[dec4,enc3]))
	g_bn12 = batch_norm(self.batch_size,name='g_bn12')
	dec5 = g_bn12(deconv2d(dec4,[self.batch_size,64,64,self.df_dim*4],name='g_dec5'),train=False)#output 64x64
	dec5 = tf.nn.relu(tf.concat(3,[dec5,enc2]))
	g_bn13 = batch_norm(self.batch_size,name='g_bn13')
	dec6 = g_bn13(deconv2d(dec5,[self.batch_size,128,128,self.df_dim*2],name='g_dec6'),train=False)#output 128x128
	dec6 = tf.nn.relu(tf.concat(3,[dec6,enc1]))
	dec7 = deconv2d(dec6,[self.batch_size,256,256,3],name='g_dec7')#output 256x256
