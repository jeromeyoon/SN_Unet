...
class networks(object):
	...
	#### encoder ####
        enc0 = tf.nn.relu(conv2d(nir,self.df_dim,name='g_enc0'))
	g_bn1 = batch_norm(self.batch_size,name='g_bn1')
        enc1 =tf.nn.relu(g_bn1(conv2d(enc0,self.df_dim*2,k_h=3,k_w=3,d_h=2,d_w=2,name='g_enc1'))) #output:128x128
	g_bn2 = batch_norm(self.batch_size,name='g_bn2')
        enc2 =lrelu(g_bn2(conv2d(enc1,self.df_dim*2,k_h=3,k_w=3,d_h=2,d_w=2,name='g_enc2'))) #output:64x64
	g_bn3 = batch_norm(self.batch_size,name='g_bn3')
        enc3 =lrelu(g_bn3(conv2d(enc2,self.df_dim*4,k_h=3,k_w=3,d_h=2,d_w=2,name='g_enc3'))) #output:32x32
	g_bn4 = batch_norm(self.batch_size,name='g_bn4') 
        enc4 =lrelu(g_bn4(conv2d(enc3,self.df_dim*4,k_h=3,k_w=3,d_h=2,d_w=2,name='g_enc4'))) #output:16x16
	g_bn5 = batch_norm(self.batch_size,name='g_bn5')
        enc5 =lrelu(g_bn5(conv2d(enc4,self.df_dim*4,k_h=3,k_w=3,d_h=2,d_w=2,name='g_enc5'))) #output:8x8
	#### Deconder #####
	...
	dec1_3 = tf.nn.relu(tf.concat(3,[dec1_1,enc4,dec1_2]))
	...
	dec2_3 = tf.nn.relu(tf.concat(3,[dec2_1,enc3,dec2_2]))
	...
	dec3_3 = tf.nn.relu(tf.concat(3,[dec3_1,enc2,dec3_2]))
	...
	dec4_3 = tf.nn.relu(tf.concat(3,[dec4_1,enc1,dec4_2]))
	...
	dec5_2 = tf.nn.relu(tf.concat(3,[dec5_1,enc0]))
	...
	#### encoder ####
        enc0 = tf.nn.relu(conv2d(nir,self.df_dim,name='g_enc0'))
	g_bn1 = batch_norm(self.batch_size,name='g_bn1')
        enc1 =tf.nn.relu(g_bn1(conv2d(enc0,self.df_dim*2,k_h=3,k_w=3,d_h=2,d_w=2,name='g_enc1'),train=False)) #output:128x128
	g_bn2 = batch_norm(self.batch_size,name='g_bn2')
