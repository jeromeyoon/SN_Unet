...
class DCGAN(object):
	...
	self.L1_2 = tf.div(tf.reduce_sum(tf.square(tf.sub(self.upsample2,tf.image.resize_nearest_neighbor(self.normal_images,[32,32])))),32*32*3)
	self.L1_3 = tf.div(tf.reduce_sum(tf.square(tf.sub(self.upsample3,tf.image.resize_nearest_neighbor(self.normal_images,[64,64])))),64*64*3)
	self.L1_4 = tf.div(tf.reduce_sum(tf.square(tf.sub(self.upsample4,tf.image.resize_nearest_neighbor(self.normal_images,[128,128])))),128*128*3)
	# generated surface normal
	# flip the labels
	if np.random.uniform(0.0,1.0,size=1)>0.3:
            self.d_loss_real = binary_cross_entropy_with_logits(tf.ones_like(self.D), self.D)
            self.d_loss_fake = binary_cross_entropy_with_logits(tf.zeros_like(self.D_), self.D_)
	else:
            self.d_loss_real = binary_cross_entropy_with_logits(tf.zeros_like(self.D), self.D)
            self.d_loss_fake = binary_cross_entropy_with_logits(tf.ones_like(self.D_), self.D_)
